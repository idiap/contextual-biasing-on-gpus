...
class BatchedThreadedNnet3CudaOnlinePipeline {
 public:
  ...
  BatchedThreadedNnet3CudaOnlinePipeline(
      const BatchedThreadedNnet3CudaOnlinePipelineConfig &config,
      const fst::Fst<fst::StdArc> &decode_fst,
      fst::Fst<fst::StdArc> &rescore_fst,  // GPU boosting
      const nnet3::AmNnetSimple &am_nnet, const TransitionModel &trans_model)
      : config_(config),
        max_batch_size_(config.max_batch_size),
        trans_model_(&trans_model),
    ...
    int num_worker_threads = config_.num_worker_threads;
    thread_pool_.reset(new ThreadPoolLight(num_worker_threads));
    rescore_fst_ = &rescore_fst;  // GPU boosting

    Initialize(decode_fst, rescore_fst);
    //Initialize(decode_fst);
  }
  ...
  BatchedThreadedNnet3CudaOnlinePipeline(
    ...
    Initialize(decode_fst, ngram_size, rescore_str);
  }
 ...
 private:
  ...
  void Initialize(const fst::Fst<fst::StdArc> &decode_fst, const fst::Fst<fst::StdArc> &rescore_fst);
  void Initialize(const fst::Fst<fst::StdArc> &decode_fst, uint32 ngram_size, std::vector<std::vector<uint32> > &rescore_str);
  ...
  void AllocateAndInitializeData(const fst::Fst<fst::StdArc> &decode_fst,
                                  uint32 ngram_size, std::vector<std::vector<uint32> > &rescore_str);
  ...
  fst::Fst<fst::StdArc> *rescore_fst_ = NULL;  // GPU boosting
