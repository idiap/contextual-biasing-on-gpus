...
#define KALDI_CUDA_DECODER_WAIT_FOR_CHANNEL_US 1000
...
void BatchedThreadedNnet3CudaOnlinePipeline::Initialize(
  ...
  ReadParametersFromModel();
  AllocateAndInitializeData(decode_fst, rescore_fst);
}

void BatchedThreadedNnet3CudaOnlinePipeline::Initialize(const fst::Fst<fst::StdArc> &decode_fst, uint32 ngram_size, std::vector<std::vector<uint32> > &rescore_str) {
  ReadParametersFromModel();
  AllocateAndInitializeData(decode_fst, ngram_size, rescore_str);
}

void BatchedThreadedNnet3CudaOnlinePipeline::AllocateAndInitializeData(
    const fst::Fst<fst::StdArc> &decode_fst,
    uint32 ngram_size,
    std::vector<std::vector<uint32> > &rescore_str
    ) {
  ...
  cuda_fst_->Initialize(decode_fst, ngram_size, rescore_str, trans_model_);
...
void BatchedThreadedNnet3CudaOnlinePipeline::AllocateAndInitializeData(
  ...
  // Decoder
  cuda_fst_ = std::make_shared<CudaFst>();
  cuda_fst_->Initialize(decode_fst, rescore_fst, trans_model_);
  cuda_decoder_.reset(new CudaDecoder(*cuda_fst_, config_.decoder_opts,
                                      max_batch_size_, config_.num_channels));
  if (config_.num_decoder_copy_threads > 0) {
    cuda_decoder_->SetThreadPoolAndStartCPUWorkers(
  ...
  for (size_t i = 0; i < is_first_chunk.size(); ++i) {
    ...
    if (channel_info.must_reset_decoder) {
      ...
      while (!channel_info.can_reset_decoder.load()) {
        usleep(KALDI_CUDA_DECODER_WAIT_FOR_CHANNEL_US);
      }
...
void BatchedThreadedNnet3CudaOnlinePipeline::FinalizeDecoding(int32 ichannel) {
  ...
  while (true) {
    ...
    Lattice lat;
    // fst::VectorFst<StdArc> lat_fst;  // GPU boosting line
    // fst::VectorFst<StdArc> composed_fst;  // GPU boosting line
...
/*        if(rescore_fst_) {
          SetResultUsingLattice(dlat, callback_w_options->result_type,
                                lattice_postprocessor_, *rescore_fst_, &result);  // GPU boosting line
        }
        else {*/
        SetResultUsingLattice(dlat, callback_w_options->result_type,
        ...
        (callback)(params);
        //bool partial = 0;
        //bool endpoint_detected = 1;
        //callback->second("Check point", partial, endpoint_detected);
      }
    }
