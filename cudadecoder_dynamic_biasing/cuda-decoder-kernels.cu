...
// mrs::  each thread processes all lanes.
__global__ void init_decoding_on_device_kernel(DeviceParams cst_dev_params,
  ...
  KALDI_CUDA_DECODER_BATCH_KERNEL_LOOP(ilane, nlanes) {
      // the command above effectively does:
      // for (int ilane = blockIdx.y; i < nlanes; i += gridDim.y)
      // I think blockIdx.y is the lane id. so it doesn't make sense to skip by gridDim.y which is the
      // number of lanes. This makes sense only when gridDim.y < nlanes
    KALDI_CUDA_DECODER_1D_KERNEL_LOOP(idx, init_main_q_end) {
        // the command above effectively does
        // for(int idx = blockIdx.x * blockDim.x + threadIdx.x; idx < init_main_q_end; idx += blockDim.x * gridDim.y)
        // blockDim.x is 256; gridDim.y is the number of lanes
        // for each ilane, process the state of 
...
__global__ void reset_for_frame_and_estimate_cutoff_kernel(
  ...
  KALDI_CUDA_DECODER_BATCH_KERNEL_LOOP(ilane, nlanes) {
    ...
    if (threadIdx.x < narcs) {
      ...
      CostType arc_fixed_cost = cst_dev_params.d_arc_weights[iarc]; // + binsearch_maxle(boost_graph_idx, iarc); // find if we need to boost this arc
      // to get boosting cost, first get number of indices
      CostType boost_cost = 0;
      // we always assume there is a boosting graph
      uint32 boost_cost_idx_max = cst_dev_params.d_boosted_arc_idx_[0];
      if (boost_cost_idx_max>0) {
          unsigned int boost_cost_idx = binsearch_uint(cst_dev_params.d_boosted_arc_idx_, iarc, 1, boost_cost_idx_max); 
          if(iarc == cst_dev_params.d_boosted_arc_idx_[boost_cost_idx]) {
                  boost_cost = -2.0;
          }
      }
      // CostType boost_cost = discount_value[rescore_fst_idx][iarc]; // 0 or -2.
      ...
      total_cost = previous_cost + arc_fixed_cost + acoustic_cost + boost_cost;
                     // +0.0f, best prev cost is normalized to 0
    }
  ...
  KALDI_CUDA_DECODER_BATCH_KERNEL_LOOP(ilane, nlanes) {
    ...
    KALDI_CUDA_DECODER_1D_BLOCK_OFFSET_KERNEL_LOOP(block_offset, thread_idx,
      ...
      if (main_q_arc_index < total_narcs) {
        ...
        uint32 boost_cost_idx_max = cst_dev_params.d_boosted_arc_idx_[0];
        if (boost_cost_idx_max>0) {
            unsigned int boost_cost_idx = binsearch_uint(cst_dev_params.d_boosted_arc_idx_, arc_idx, 1, boost_cost_idx_max); 
            if(arc_idx == cst_dev_params.d_boosted_arc_idx_[boost_cost_idx]) {
                boost_cost = -2.0;
            }
        }
        const CostType arc_fixed_cost = cst_dev_params.d_arc_weights[arc_idx] + boost_cost;
    ...
    while (main_q_narcs > 0) {
      ...
      KALDI_CUDA_DECODER_1D_BLOCK_OFFSET_KERNEL_LOOP(offset, thread_idx,
        ...
        if (main_q_arc_idx < main_q_narcs) {
          ...
          CostType arc_weight = cst_dev_params.d_arc_weights[arc_idx] + boost_cost;
