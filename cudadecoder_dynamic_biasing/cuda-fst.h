...
class CudaFst {
 ...
 private:
  ...
  void PopulateArcs(const fst::Fst<StdArc> &fst, const fst::Fst<fst::StdArc> &rescore_fst);
  void PopulateArcs(const fst::Fst<StdArc> &fst,
                           uint32 ngram_size,
  ...
  std::vector<std::vector <uint32> > h_boosted_arc_idx_;
  CostType *d_arc_weights_;
  // for each boosting graph, store the list of weights.
  CostType discount_factor;
  // for each boosting graph, store the list of arcs that need to be boosted
  uint32 * d_boosted_arc_idx_;
